...
""",
    ...
    def test_validate_gates_dual_review_all_pass_allows_final_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            project_root = Path(tmp) / "project"
            state_dir = project_root / "ai" / "state"
            reports_dir = project_root / "ai" / "reports"
            ...
            report = validate_gates.validate(project_root)
            self.assertTrue(report["dual_review_gate_ready"])
            self.assertTrue(report["final_gate_passed"])

    def test_validate_gates_dual_review_fail_then_pass_blocks_final_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            project_root = Path(tmp) / "project"
            state_dir = project_root / "ai" / "state"
            reports_dir = project_root / "ai" / "reports"
                ...
                json.dumps(
                    {
                        ...
                        "review_conflict": True,
                        "review_arbitration_required": True,
                        "review_arbitration_status": "pending",
                        "review_arbitration_evidence": "",
                        "review_pass_1_run_id": "run-001",
                        "review_pass_2_run_id": "run-001",
                        "review_pass_1_commit_sha": "abc123",
