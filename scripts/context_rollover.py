...
def build_rollover_payload(project_root: Path, agent_id: str = "orchestrator") -> dict:
...
5. Read the latest ai/reports/orchestrator-rollover.md
6. Dispatch the next ready workflow step or resolve blockers
"""
    return {
        "agent_id": agent_id,
        "created_at": utc_now(),
...
def create_rollover(project_root: Path, agent_id: str = "orchestrator") -> dict:
    payload = build_rollover_payload(project_root, agent_id=agent_id)
